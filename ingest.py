...
BAR_PATH = './data/media/collections/ocr/CH-BAR_E8100C_1999-365_86/CH-BAR_E8100C_1999-365_86_7/'
BAR_NUM_VOLUMES = 107
...
def bar_load_documents(base_path):
    for doc_path in base_path.glob('*.hocr'):
        hocr = doc_path.read_text()
        yield {'id': doc_path.stem.split(".")[0],
               'source': 'bar',
               'ocr_text': '/data/media/collections/ocr/CH-BAR_E8100C_1999-365_86/CH-BAR_E8100C_1999-365_86_7/' + doc_path.name,
               'author': 'Werner Christof Roth',
               'title': 'Der Vereina-Eisenbahntunnel im Lichte der Erfahrungen zum Furkatunnel',
               'date': '1985-01-01T00:00:00Z',
               'language': 'DE',
               'publisher': 'Hochschule St.Gallen'}
...
if __name__ == '__main__':
    with ProcessPoolExecutor(max_workers=8) as pool:
        ...
        #    fut.result()
        futs = []
        print("\nIndexing BAR volumes")
        bar_iter = bar_load_documents(Path(BAR_PATH))
        for idx, batch in enumerate(generate_batches(bar_iter, 4)):
            futs.append(pool.submit(index_documents, batch))
            print("\r{:04}/{}".format((idx+1)*4, BAR_NUM_VOLUMES), end='')
        for fut in as_completed(futs):
            fut.result()
    print("\n")
