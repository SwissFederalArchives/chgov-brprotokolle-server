...
export function getAccess(collectionId: string, marc: Document): string {
    const marc542m = marc.get('//marc:datafield[@tag="542"]/marc:subfield[@code="m"]', ns) as Element;
    if (marc542m)
...
function extractAuthors(marc: Element, metadata: MARCXMLMetadata): void {
    const authors = [
        ...
        .map(({marcAuthor, role}) => {
            ...
            const bElem = marcAuthor.get('./marc:subfield[@code="b"]', ns) as Element;
            if (bElem)
                name = name + ' ' + normalize(bElem.text().trim());

            const cElem = marcAuthor.get('./marc:subfield[@code="b"]', ns) as Element;
            if (cElem)
                name = name + ' ' + normalize(cElem.text().trim());

            const dElem = marcAuthor.get('./marc:subfield[@code="b"]', ns) as Element;
            if (dElem)
                name = name + ' ' + normalize(dElem.text().trim());

            const marcAuthorsType = marcAuthor.get('./marc:subfield[@code="e"]', ns) as Element;
...
function extractHdlToMetadata(marc: Element, metadata: MARCXMLMetadata): void {
    const metadataHdl = marc.get('//marc:datafield[@tag="902"]', ns) as Element;
    if (metadataHdl)
...
function extractSignature(marc: Element, collectionId: string, metadata: MARCXMLMetadata): void {
    const marc852 = (marc.find('//marc:datafield[@tag="852"]', ns) as Element[]).find((marc852): any => {
        const marc852p = marc852.get('./marc:subfield[@code="p"]', ns) as Element;
        return marc852p && marc852p.text().trim() === collectionId;
    });

    if (marc852) {
        const marc852c = marc852.get('./marc:subfield[@code="c"]', ns) as Element;
        const marc852j = marc852.get('./marc:subfield[@code="j"]', ns) as Element;
