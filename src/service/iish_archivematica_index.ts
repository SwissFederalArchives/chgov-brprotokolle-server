...
export default async function processDip({collectionPath}: IndexParams): Promise<void> {
    try {
        ...
        const _e = e as any;
        const err = new Error(`Failed to index the collection ${collectionPath}: ${_e.message}`);
        err.stack = _e.stack;
        throw err;
    }
}
...
export async function processCollection(collectionPath: string): Promise<CollectionProcessingResult> {
    ...
    const rootLogical = mets.get('//mets:structMap[@ID="structMap_2"]/mets:div/mets:div', ns) as Element;
    const rootPhysical = mets.get('//mets:structMap[@TYPE="physical"]/mets:div/mets:div', ns) as Element;
    const rootStructureIISH = mets.get('//mets:structMap[@ID="structMap_iish"]/mets:div', ns) as Element;
    if (!rootPhysical)
...
function getRootItem(mets: Document, structureIISH: Element | null): Item {
    const rootDir = mets.get('//mets:structMap[@TYPE="physical"]/mets:div', ns) as Element;
    if (!rootDir)
        throw new Error('Could not find the physical structmap in the METS file');

    const rootDmdIdAttr = rootDir.attr('DMDID');
    const rootDmdId = rootDmdIdAttr ? rootDmdIdAttr.value() : null;
    const premisObj = mets.get(`//mets:dmdSec[@ID="${rootDmdId}"]/mets:mdWrap/mets:xmlData/premisv3:object`, ns) as Element;
    if (!premisObj)
        throw new Error('Could not find the premis object of the root item in the METS file');

    const uuidElem = premisObj.get('./premisv3:objectIdentifier/premisv3:objectIdentifierType[text()="UUID"]/../premisv3:objectIdentifierValue', ns) as Element;
    const originalNameElem = premisObj.get('./premisv3:originalName', ns) as Element;
...
function walkTree({id, mets, objects, relativeRootPath, curNode, curNodePhysical, structureIISH, parent = null}:
    ...
    nodes.forEach(node => {
        ...
        const nodePhysical : Element = curNodePhysical.get(`./mets:div[@LABEL="${label}"]`, ns) || curNodePhysical;
...
function readFolder(rootId: string, label: string, mets: Document, node: Element,
    ...
    const premisObj = mets.get(`//mets:dmdSec[@ID="${dmdId}"]/mets:mdWrap/mets:xmlData/premisv3:object`, ns) as Element;
    if (!premisObj)
...
function readFile(rootId: string, label: string, mets: Document, objects: string[], relativeRootPath: string,
    ...
    const fptrElem = node.get('mets:fptr', ns) as Element;
    ...
    const originalNameElem = premisObj.get(`./premis:originalName`, ns) as Element;
    if (!originalNameElem)
    ...
    const objCharacteristics = premisObj.get('./premis:objectCharacteristics', ns) as Element;
    const objCharacteristicsExt = objCharacteristics
        ? objCharacteristics.get('./premis:objectCharacteristicsExtension', ns) as Element: null;
    ...
    const sizeElem = objCharacteristics.get('./premis:size', ns) as Element;
    const size = sizeElem ? parseInt(sizeElem.text()) : null;

    const dateCreatedByAppElem = objCharacteristics.get('.//premis:creatingApplication/premis:dateCreatedByApplication', ns) as Element;
    const creationDate = dateCreatedByAppElem
        ? moment(dateCreatedByAppElem.text(), 'YYYY-MM-DD').toDate() : null;

    const pronomKeyElem = objCharacteristics.get('./premis:format/premis:formatRegistry/premis:formatRegistryName[text()="PRONOM"]/../premis:formatRegistryKey', ns) as Element;
    ...
    let order = null;
    if (structureIISH) {
        const pageDiv = structureIISH.get(`./mets:div[@TYPE="page"]/mets:fptr[@FILEID="${fileId}"]/..`, ns) as Element;
...
function readText(rootId: string, label: string, mets: Document, objects: string[], relativeRootPath: string,
    ...
    const fptr = fptrs.find(fptrElem => {
        ...
        const parentFolderDiv = mets.get(`//mets:structMap[@TYPE="physical"]//mets:fptr[@FILEID="${itemFileId}"]/../..`, ns) as Element;
        if (parentFolderDiv) {
...
function findPremisObj(mets: Document, fileId: string): Element | null {
    const fileNode = mets.get(`mets:fileSec/mets:fileGrp[@USE="original"]/mets:file[@ID="${fileId}"]`, ns) as Element;
...
export function getIdentifier(premisObj: Element, premisNS: 'premis' | 'premisv3' = 'premis'): string | null {
    const hdlObj = premisObj.get(`./${premisNS}:objectIdentifier/${premisNS}:objectIdentifierType[text()="hdl"]`, ns)as Element;
    const uuidObj = premisObj.get(`./${premisNS}:objectIdentifier/${premisNS}:objectIdentifierType[text()="UUID"]`, ns)as Element;

    if (hdlObj) {
        const objIdAttr = hdlObj.get(`./../${premisNS}:objectIdentifierValue`, ns)as Element;
        if (objIdAttr) {
    ...
    if (uuidObj) {
        const objIdAttr = uuidObj.get(`./../${premisNS}:objectIdentifierValue`, ns)as Element;
        if (objIdAttr)
...
export function determineResolution(objCharacteristicsExt: Element): { width: number | null; height: number | null } {
    const mediaInfo = objCharacteristicsExt.get('./mediainfo:MediaInfo/mediainfo:media/mediainfo:track[@type="Image" or @type="Video"]', ns)as Element;
    if (mediaInfo) {
        const widthElem = mediaInfo.get('./mediainfo:Width', ns)as Element;
        const heightElem = mediaInfo.get('./mediainfo:Height', ns)as Element;
    ...
    const ffprobe = objCharacteristicsExt.get('./ffprobe/streams/stream[@codec_type="video"]', ns)as Element;
    if (ffprobe) {
    ...
    const exifTool = objCharacteristicsExt.get('./rdf:RDF/rdf:Description', ns)as Element;
    if (exifTool) {
        const widthElem = exifTool.get('./File:ImageWidth', ns)as Element;
        const heightElem = exifTool.get('./File:ImageHeight', ns)as Element;
    ...
    const fitsExifTool = objCharacteristicsExt.get('./fits:fits/fits:toolOutput/fits:tool[@name="Exiftool"]/exiftool', ns)as Element;
    if (fitsExifTool) {
        const widthElem = fitsExifTool.get('./ImageWidth', ns)as Element;
        const heightElem = fitsExifTool.get('./ImageHeight', ns)as Element;
...
export function determineDpi(objCharacteristicsExt: Element): number | null {
    const exifTool = objCharacteristicsExt.get('./rdf:RDF/rdf:Description', ns)as Element;
    if (exifTool) {
        const resolutionElem = exifTool.get('./IFD0:XResolution', ns)as Element;
    ...
    const fitsExifTool = objCharacteristicsExt.get('./fits:fits/fits:toolOutput/fits:tool[@name="Exiftool"]/exiftool', ns)as Element;
    if (fitsExifTool) {
        const resolutionElem = fitsExifTool.get('./XResolution', ns)as Element;
...
export function determineDuration(objCharacteristicsExt: Element): number | null {
    const mediaInfo = objCharacteristicsExt.get('./mediainfo:MediaInfo/mediainfo:media/mediainfo:track[@type="General"]', ns)as Element;
    if (mediaInfo) {
        const durationElem = mediaInfo.get('./mediainfo:Duration', ns)as Element;
...
export function determineEncoding(objCharacteristicsExt: Element): string | null {
    const md = objCharacteristicsExt.get('./fits:fits/fits:toolOutput/fits:tool[@name="Tika"]/metadata', ns)as Element;
    if (md) {
        const contentEncodingValueElem = md.get('./field[@name="Content-Encoding"]/value', ns)as Element;
